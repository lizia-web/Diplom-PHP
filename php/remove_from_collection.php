...
if (!$user_id || !$work_id || !$collection_name) {
...
try {
    ...
    $stmt->execute(['user_id' => $user_id, 'name' => $collection_name]);
    ...
    $stmt = $pdo->prepare("DELETE FROM collection_works WHERE collection_id = :collection_id AND work_id = :work_id");
    $stmt->execute([
